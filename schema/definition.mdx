...
A document model represents the shape of a document in a particular Firestore collection. There are two Typesync types that can represent the shape of a document model: `object` and `map`. Use `object` when you know the exact fields that the document should contain (this is most often the case). Use `map` when the document is used as an arbitrary mapping.
...
This indicates to your IDE that the file is not just any YAML file but is part of a Typesync definition. This will allow your IDE to provide Intellisense/autocomplete for definition fields.
