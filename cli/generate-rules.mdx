...
Since Security Rules are centralized in a single file (typically `firestore.rules`), and developers often implement custom rules alongside type validations, Typesync is designed to insert only the necessary validator functions, without overriding your custom rules. You can specify where these validators are added within the file using the `startMarker` and `endMarker` options. For a detailed guide, see the full [example](#example) below.
...
<Info>
  ...
  Typesync will provide the most stringent validation possible within
  these constraints.
</Info>
