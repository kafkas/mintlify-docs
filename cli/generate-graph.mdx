...
Generates a Mermaid graph for the specified schema and injects it into the specified Markdown file. You can specify where the generated graph is inserted within the file using the `--startMarker` and `--endMarker` options. For a detailed guide, see the full [example](#example) below.
